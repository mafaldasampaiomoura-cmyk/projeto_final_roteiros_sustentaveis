...
exports.removeFavourite = exports.addFavourite = exports.listFavourites = void 0;
const supabase_1 = require("../config/supabase");
const listFavourites = async (_req, res) => {
...
exports.listFavourites = listFavourites;
const addFavourite = async (req, res) => {
    ...
    const { data: existingFavourite, error: existingFavouriteError } = await supabase_1.supabase
        .from('favourites')
        .select('id')
        .eq('user_id', user.id)
...
const removeFavourite = async (req, res) => {
    ...
    const user = res.locals.user;
    const { data: favourite, error: favouriteError } = await supabase_1.supabase
        .from('favourites')
        .select('id')
        .eq('user_id', user.id)
        .eq('route_id', routeId)
